...
import { Building2, Eye, EyeOff, UserPlus, AlertTriangle } from 'lucide-react';
...
export function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (identifier.length < 3) return setError('Identifier must be at least 3 characters.');
    ...
    try {
      ...
      const isAdmin = identifier.toLowerCase() === 'admin' || role === 'staff';
      ...
      const { data, error: authError } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            username: identifier, // Store the original identifier as username
            role,
            isAdmin
          }
        }
      ...
      <div ref={container} className="glass glass-xl w-full max-w-[440px] p-10 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="text-center mb-8">
            ...
            <h1 className="text-[1.65rem] font-extrabold text-on-surface tracking-[-0.5px] mb-1">Create account</h1>
            <p className="text-[0.9rem] text-on-surface-var">Join the campus reporting community.</p>
          </div>

          <div className="flex gap-1 p-1 bg-surface-low rounded-full mb-6">
            <button type="button" onClick={() => setRole('student')} className={`flex-1 py-2 rounded-full text-[0.85rem] font-bold transition-all ${role === 'student' ? 'bg-white text-primary shadow-sm' : 'text-on-surface-var hover:bg-white/50'}`}>Student</button>
            <button type="button" onClick={() => setRole('staff')} className={`flex-1 py-2 rounded-full text-[0.85rem] font-bold transition-all ${role === 'staff' ? 'bg-white text-primary shadow-sm' : 'text-on-surface-var hover:bg-white/50'}`}>Staff</button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div>
              <label className="block text-[0.78rem] font-bold uppercase tracking-wider text-on-surface-var mb-2">
            ...
            <div>
              <label className="block text-[0.78rem] font-bold uppercase tracking-wider text-on-surface-var mb-2">Password</label>
              <div className="relative">
                <input type={showPw ? "text" : "password"} required value={password} onChange={e => setPassword(e.target.value)} placeholder="Min. 6 characters" className="w-full bg-white border-2 border-outline-var rounded-xl p-3.5 text-[0.9rem] text-on-surface outline-none focus:border-primary-container focus:shadow-[0_0_0_4px_rgba(37,99,235,0.07)] transition-all" />
            ...
            <div>
              <label className="block text-[0.78rem] font-bold uppercase tracking-wider text-on-surface-var mb-2">Confirm Password</label>
              <div className="relative">
                <input type={showPw2 ? "text" : "password"} required value={confirmPw} onChange={e => setConfirmPw(e.target.value)} placeholder="Repeat password" className="w-full bg-white border-2 border-outline-var rounded-xl p-3.5 text-[0.9rem] text-on-surface outline-none focus:border-primary-container focus:shadow-[0_0_0_4px_rgba(37,99,235,0.07)] transition-all" />
                <button type="button" onClick={() => setShowPw2(!showPw2)} className="absolute right-3 top-1/2 -translate-y-1/2 text-outline hover:text-primary-container p-1">
